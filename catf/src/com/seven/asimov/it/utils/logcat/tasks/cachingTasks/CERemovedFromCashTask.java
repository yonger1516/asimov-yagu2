...
public class CERemovedFromCashTask extends Task <CashEntryWraper> {

    private static final String REGEXP = "(201[2-9]/[0-9]*/[0-9]*\\s[0-9]*\\:[0-9]*\\:[0-9]*\\.[0-9]*).([A-Z]*).*\\sCE\\s\\[(\\S+)\\]\\sremoved";
    ...
    protected CashEntryWraper parseLine(String line) {
        ...
        if (matcher.find()) {
            ...
            //reset singleton if new request is processed
            if (!wrapper.getCeId().equals(matcher.group(3))) {
                wrapper.setCeId(matcher.group(3));
                wrapper.setDelitedFromFS(false);
                wrapper.setDelitedFromBD(false);
            }

            wrapper.setDelitedFromCash(true);
