...
public class PunycodeUtil {
    ...
    public static String cyrillicToPunicode(String link) throws PunycodeException {
        String result = link.substring(7);
        result.trim();
        String[] values = result.split("\\.");
        String[] encodedValues = new String[values.length];
        result = "http://";
        for (int i = 0; i < values.length; i++) {
            encodedValues[i] = PunycodeUtil.encode(values[i]);
            if (i < values.length - 1)
                result = result + "xn--" + encodedValues[i] + ".";
            else
                result = result + "xn--" + encodedValues[i];
        }
        return result;
    ...
    private static class PunycodeException extends Exception {
