...
// Class was saved by request for Oleg Steblyna.
// Will be used as real usecase.
@Ignore
public class HostSendRstWhileOCProcessingRequestTest_ForSteblyna extends SimpleTest {
    ...
    public HostSendRstWhileOCProcessingRequestTest_ForSteblyna(ConnSelector selector, HttpServer server) {
    ...
    public void onDataSent(SelectableChannel channel) {
        ...
        Log.v(TAG, "isServerRstSent=" + isServerRstSent);
        /*if (mClient.getChannel().equals(channel) && mClient.hasRequestPending() && mClient.getRequestId() == 2 && !isServerRstSent) {
            try {
