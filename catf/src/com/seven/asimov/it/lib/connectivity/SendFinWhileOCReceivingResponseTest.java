 ...
 * </ul>
 */
public class SendFinWhileOCReceivingResponseTest extends SimpleTest {
    private static final String TAG = SendFinWhileOCReceivingResponseTest.class.getSimpleName();

    public SendFinWhileOCReceivingResponseTest(ConnSelector selector, HttpServer server) {
    ...
    public void onDataSent(SelectableChannel channel) {
        ...
        if (!mClient.getChannel().equals(channel) && !mClientFinSent) {
            try {
                mClient.sendFin();
                ConnLogger.debug(TAG, getName() + ": client sends FIN");
            } catch (IOException e) {
                ConnLogger.error(TAG, getName() + ": client sends FIN failed", e);
                fail(mClient.getConnId(), CLIENT, SEND_FAILED);
            }
        }
    }
