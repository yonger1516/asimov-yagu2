 ...
 * <li>2. Host sends FIN after response data before close client connection due to long poll start.
 ...
 * <li>2. OC receives FIN after response data, forward the response to App and the content is served from cache due to long poll, then sends RST to App and network side."
 * </ul>
 */
public class HostSendFinInLongPollTest extends HostSendFinInRapidPollTest {
    private static final String TAG = HostSendFinInLongPollTest.class.getSimpleName();

    public HostSendFinInLongPollTest(ConnSelector selector, String expectedBody) {
        super(TAG, selector, "asimov_it_host_send_fin_in_long_poll", expectedBody, 0);
    }
