 ...
 * <li>3. OC sends FIN to App because the response header didn't contain the content length. </li>
 * </ul>
 ...
 * <li>9. XX : CLIENT : FIN_RECEIVED</li>
 * </ul>
 */


public class CloseInResponseWithoutContentLenghtTest extends SimpleTest {
    private static final String TAG = CloseInResponseTest.class.getSimpleName();

    private boolean hasConnectionCloseInResponse;

    public CloseInResponseWithoutContentLenghtTest(ConnSelector selector, HttpServer server) {
    ...
    public void onRequestReceived(int connId, HttpRequest request) {
        ...
        try {
            ...
            HttpResponse response = buildSimpleResponse(request, false);
    ...
    public void onDataSent(SelectableChannel channel) {
        ...
        if (connId != -1) {
            try {
                ...
                addResult(connId, TcpConnection.ConnType.SERVER, FIN_SENT);
            } catch (IOException e) {
                ConnLogger.error(TAG, getName() + ": Close server socket failed", e);
                this.fail(connId, TcpConnection.ConnType.SERVER, FIN_SENT);
            }
        }
    }
    ...
    public void onFinReceived(SelectableChannel channel) {
        ...
        if (isServerReceivedRequest && isClientReceivedResponse && hasConnectionCloseInResponse && isClientReceivedFin) {
            pass();
        }
    }
    ...
    protected void onDone(Status status) {
        ...
        if (status == Status.TIMEDOUT) {
            ...
            } else if (!isClientReceivedFin) {
                this.fail("client didn't receive FIN");
            }
        }
    }
